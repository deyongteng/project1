...
var bgmImg=document.getElementsByClassName("bgmImg")[0];
var bgm=document.getElementsByClassName("bgm")[0];
var JQ_bgmImg=$(".bgmImg").eq(0);
function BGM(){
	var off=true;
	$(".bgm").eq(0).attr("src",audioM[0]);
	bgm.play();
	bgm.loop="loop";
	bgmImg.addEventListener("touchstart",function(ev){
		ev.stopPropagation();
		if(off){
			bgm.pause();
...
diWuPingHtml+="<ul class='bgBox5'>";
	for (var i = 0; i < diWuPing.length; i++) {
		...
		if(i===11){
			...
			diWuPingHtml+="<a href='https://coding.net/u/JavaScript_TDY/p/project/git'>"+diWuPing[i]+"</a>";
			diWuPingHtml+="</li>";
		}else if(i===9){
			diWuPingHtml+="<li class='bg5_"+(i+1)+"Box''>";
...
jQ_animatiBox.on("touchstart",function(ev){
	height=document.getElementsByClassName("animatiBox")[0].offsetHeight;
